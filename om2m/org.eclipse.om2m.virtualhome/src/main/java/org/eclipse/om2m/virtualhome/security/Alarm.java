...
public class Alarm {
	...
	public static void createAlarmResources(){
	    ...
	    if(response.getResponseStatusCode().equals(ResponseStatusCode.CREATED)){
				...
				content = ObixUtil.getAlarmDataRep(actuatorValue);
				RessourceManager.createDataContentInstance(appId, DATA, content);
				
				// Create the DESCRIPTOR contentInstance
				content = ObixUtil.getAlarmDescriptorRep(appId, Monitor.ipeId);
	...
	public static class AlarmListener extends Thread{
			...
			public void run() {
				while(running){
					...
					if(memorizedActuatorValue != actuatorValue){
						...
						String content = ObixUtil.getAlarmDataRep(actuatorValue);
						RessourceManager.createDataContentInstance(appId, DATA, content);
					}
	...
	public static ResponsePrimitive AlarmController(String valueOp, ResponsePrimitive responsein) {
		ResponsePrimitive response = responsein;
		switch(valueOp){
		case "get":
			response.setContent(ObixUtil.getAlarmDataRep(actuatorValue));
