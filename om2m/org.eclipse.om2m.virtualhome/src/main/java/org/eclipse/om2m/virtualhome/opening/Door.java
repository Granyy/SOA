...
public class Door {
	...
	public static void createDoorResources(){
	    ...
	    if(response.getResponseStatusCode().equals(ResponseStatusCode.CREATED)){
				...
				content = ObixUtil.getDoorDataRep(actuatorValue);
	...
	public static class DoorListener extends Thread{
			...
			public void run() {
				while(running){
					...
					if(memorizedActuatorValue != actuatorValue){
						...
						String content = ObixUtil.getDoorDataRep(actuatorValue);
						RessourceManager.createDataContentInstance(appId, DATA, content);
					}
	...
	public static ResponsePrimitive DoorController(String valueOp, ResponsePrimitive responsein) {
		...
		switch(valueOp){
		case "get":
			response.setContent(ObixUtil.getDoorDataRep(actuatorValue));
