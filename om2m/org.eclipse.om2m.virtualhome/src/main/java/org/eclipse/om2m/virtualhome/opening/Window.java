...
public class Window {
	...
	public static void createWindowResources(){
		String content;
	    // Create the AE
	    ...
	    if(response.getResponseStatusCode().equals(ResponseStatusCode.CREATED)){
				...
				content = ObixUtil.getWindowDataRep(actuatorValue);
				RessourceManager.createDataContentInstance(appId, DATA, content);
				
				// Create the DESCRIPTOR contentInstance
				content = ObixUtil.getWindowDescriptorRep(appId, Monitor.ipeId);
	...
	public static class WindowListener extends Thread{
			...
			public void run() {
				while(running){
					...
					if(memorizedActuatorValue != actuatorValue){
						...
						String content = ObixUtil.getWindowDataRep(actuatorValue);
						RessourceManager.createDataContentInstance(appId, DATA, content);
					}
	...
	public static ResponsePrimitive WindowController(String valueOp, ResponsePrimitive responsein) {
		ResponsePrimitive response = responsein;
		switch(valueOp){
		case "get":
			response.setContent(ObixUtil.getWindowDataRep(actuatorValue));
